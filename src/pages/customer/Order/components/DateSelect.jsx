...
function DateSelect({setStatus}) {
  ...
  const options = [
    ...
    {
      id: 1,
      period: 'Đơn đang thuê'
    },
    {
      id: 2,
      period: 'Đơn đã hoàn thành'
    },
    {
      id: 3,
      period: 'Đơn bị quá hạn'
    },
    {
      id: 4,
      period: 'Tất cả đơn hàng'
    }
  ];
  ...
  return (
    <div className="relative">
      <button
        ref={trigger}
        className="btn justify-between w-full h-[50px] mb-5 bg-white border-slate-200 hover:border-slate-300 text-slate-500 hover:text-slate-600"
      ...
      <Transition
        show={dropdownOpen}
        tag="div"
        className="z-10 absolute top-full right-0 w-[200px] bg-white border border-slate-200 py-1.5 rounded shadow-lg overflow-hidden mt-1"
            ...
            options.map(option => {
              return (
                <button
                  ...
                  onClick={() => { setSelected(option.id); setDropdownOpen(false); setStatus(option?.id)}}
                >
